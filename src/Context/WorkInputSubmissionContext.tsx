...
interface WorkSubmission {
  ...
  WorkSelectTask: (worktask: WorkTask | null) => void;
...
export const WorkTaskProvider: React.FC<{ children: React.ReactNode }> = ({
  children,
}) => {
  const [WorkTasks, setWorkTasks] = useState<{ [day: string]: WorkTask[] }>({});
  // update a state with a interface Task structure
  const [WorkSelectedTask, setWorkSelectedTask] = useState<WorkTask | null>(
    null
  );

  const [subtaskCompletion, setSubtaskCompletion] = useState<{
  ...
  const toggleWorkCompleted = (day: string, index: number) => {
    setWorkTasks((prev) => ({
      ...
      [day]: prev[day].map(
        (task, i) =>
          i === index ? { ...task, completed: !task.completed } : task // unchanged task
      ),
    }));
  };

  const WorkSelectTask = (task: WorkTask | null) => {
  ...
  const WorkToggleSubTask = (
    day: string,
    taskIndex: number,
    subIndex: number
  ) => {
    if (!WorkTasks[day] || !WorkTasks[day][taskIndex]) return;
